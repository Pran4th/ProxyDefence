...
async def get_leaderboard(
    ...
    try:
        entries = await _leaderboard.get_rankings(
            metric=metric, model_type=model_type, limit=limit, offset=offset, pool=pool,
        )
...
async def get_top_models(
    ...
    try:
        entries = await _leaderboard.get_top_n(metric=metric, n=n, model_type=model_type, pool=pool)
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
    return {
        "rankings": [{
...
async def get_model_history(
    ...
    try:
        entries = await _leaderboard.get_model_history(model_name, pool=pool)
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
    return {
...
@router.post("/compare")
async def compare_leaderboard_entries(
    body: dict,
    pool: asyncpg.Pool = Depends(get_pool),
) -> dict[str, Any]:
    ...
    try:
        await _leaderboard.refresh_from_db(pool=pool)
...
async def add_ranking_entry(
    ...
    try:
        entry_id = await _leaderboard.add_entry(entry, pool=pool)
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
