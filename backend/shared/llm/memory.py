...
from backend.shared.memory.agent import AgentMemory, AgentMemoryStore, agent_memory_store
...
class ConversationMemory:
    ...
    Delegates to backend.shared.memory for specialized memory types:
    - ExecutionMemory: tracks execution plan history
    - AgentMemory: per-agent isolated memory
    - ContextCompressor: summarization and compression
    """
    ...
    def __init__(self, max_messages: int = 50, max_tokens: int = 32000):
        ...
        self._compressor = ContextCompressor(max_tokens=max_tokens)
    ...
    @property
    def execution_memory(self) -> ExecutionMemory:
        return self._execution_memory
    ...
    def get_agent_memory(self, agent_name: str) -> AgentMemory:
        return agent_memory_store.get(str(id(self)), agent_name)
